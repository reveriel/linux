...
static inline int pksm_flags_can_scan(unsigned long vm_flags)
{
	return !(vm_flags & (VM_PFNMAP | VM_IO  | VM_DONTEXPAND |
				  (VM_DONTEXPAND | VM_DONTDUMP) | VM_HUGETLB |
#ifdef VM_SAO
			...
			unsigned long *vm_flags)
{
	...
	if (!rmap_item)
		return 0;
again:
	hlist_for_each_entry_safe(stable_anon, hlist, &rmap_item->hlist, hlist) {
...
//		list_for_each_entry(vmac, &anon_vma->head, same_anon_vma) {
		anon_vma_interval_tree_foreach(vmac, &anon_vma->rb_root,
			...
			referenced += page_referenced_one_check(page, vma,
				rmap_item->address, &mapcount);
...
//		list_for_each_entry(vmac, &anon_vma->head, same_anon_vma) {
		anon_vma_interval_tree_foreach(vmac, &anon_vma->rb_root,
			...
			ret = try_to_unmap_one_pksm(page, vma,
					rmap_item->address, NULL);
